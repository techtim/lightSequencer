...
void ofxMidiIn::openPortByName(string portName){
...
unsigned int ofxMidiIn::getPortByName(string portName){
    for(unsigned int i=0; i<nPorts; i++){
        size_t found = midii.getPortName(i).find(portName);
        if (found != string::npos) return i;
    }
    printf("MIDI IN Port with name: \"%s\" not found", portName.c_str());
    return 100;
}
// --------------------------------------------------------------------------------------
