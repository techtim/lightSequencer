...
*/
 ofSetHexColor(0xFF0066);
	drawVector(aubioOnsetDetectionVector, aubioOnsetDetectionVector.size()-1);//-(scrollWidth/2)
//,  aubioOnsetDetectionVector.size()-1
	...
	drawVector(aubioMedianVector, (int)aubioOnsetDetectionVector.size()-1);
	drawOnsetEvents(aubioOnsetDetectionVector.size()-1);
	
	

	ofSetHexColor(0x0000FF);
	drawBeatStripes(aubioOnsetDetectionVector.size()-1);
...
void LiveAudioAnalysis::drawBeatStripes(int minIndex){
	...
	float screenWidth = width;//ofGetWidth();  
	float screenHeight = height;//ofGetHeight() ;
	...
	for (int j = 0;beatIndex + j < beatFrames.size() && beatFrames[beatIndex + j] < startingFrame + scrollWidth;j++){
		...
		int screenPosition = x + (beatStripePosition - startingFrame)*screenWidth/scrollWidth;
		ofLine(x+screenPosition, y, x+screenPosition, y+screenHeight);
		if (beatIndex + j - 1 >= 0)
			ofDrawBitmapString(ofToString(beatFrames[beatIndex+j] - beatFrames[beatIndex+j-1]), x+screenPosition+ 5, y+30);
		
		if (beatIndex + j > 3){
			double bpm = framesToSeconds((beatFrames[beatIndex + j] - beatFrames[beatIndex+j-4])/4.);//
			bpm = secondsToBPM(bpm);
			ofDrawBitmapString(ofToString(bpm, 2), x+screenPosition+ 5, y+90);
		}
	...
	if ((aubioOnsetDetectionVector.size() - lastBeatTime) < 5){
		...
		ofCircle(x+screenWidth/2, y+20, 20);
	}
...
void LiveAudioAnalysis::drawVector(DoubleVector& energyVec, int minIndex){
	...
	for (i = 0; i < scrollWidth - 1; i++){
		...
		if (j < energyVec.size()){
			xPosition = x+i*screenWidth/scrollWidth;
			newHeight =	y + height - (energyVec[j]*height/heightFactor);
		
			//draw the tops
			ofLine(xPosition+x, y + height - (energyVec[j]*height/heightFactor),
				   x+screenWidth*(i+1)/scrollWidth, y + height - (energyVec[j]*height/heightFactor));

			ofLine(x+xPosition,lastHeight, x+xPosition, newHeight);
